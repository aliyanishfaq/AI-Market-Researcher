...
from deep_research.web_search import Firecrawl
...
async def process_serp_result(
    ...
    """Process search results to extract detailed learnings with citations."""
    
    contents = [
        trim_prompt(item.get("markdown", ""), 20_000)
    ...
    prompt = (
        ...
        f"generate highly detailed learnings from the contents. Return a JSON object with 'learnings' "
        f"and 'followUpQuestions' keys with array of strings as values. Include up to {num_learnings} learnings and "
        f"{num_follow_up_questions} follow-up questions.\n\n"
        f"LEARNING REQUIREMENTS:\n"
        f"1. Each learning should be a detailed paragraph (4-6 sentences minimum)\n"
        f"2. Include ALL specific numbers, metrics, dates and technical details found\n"
        f"3. Add inline citations to each claim in [SourceName] format\n"
        f"4. Combine related information from multiple sources into coherent points\n"
        f"5. Include exact quotes where particularly relevant\n"
        f"6. Don't summarize - capture all granular details\n"
        f"7. Focus on factual, data-backed insights rather than general statements\n"
        f"8. When sources conflict, note the discrepancies\n\n"
...
async def write_final_report(
    ...
    user_prompt = (
    f"As a senior market research analyst, create a comprehensive bottom-up market analysis that adheres to professional industry standards. "
    f"Return a JSON object with a 'reportMarkdown' field containing a detailed markdown report (at least 1000 words) that follows this structure:\n\n"
    f"REPORT STRUCTURE:\n"
    f"1. Market Size & Industry Analysis\n"
    f"   - Total addressable market (TAM) with specific numbers\n"
    f"   - Market segmentation with clear data points\n"
    f"   - Industry growth rates and projections\n"
    f"   - Key market drivers with supporting data\n\n"
    f"2. Bottom-Up Analysis\n"
    f"   - Detailed quantitative analysis of survey responses\n"
    f"   - Clear segmentation based on response patterns\n"
    f"   - Statistical analysis of response distributions\n"
    f"   - Data-backed persona insights and quotes\n\n"
    f"3. Competitive Landscape\n"
    f"   - Market share data and trends\n"
    f"   - Detailed competitor product mapping\n"
    f"   - Price-performance analysis across segments\n"
    f"   - Competitive positioning matrix\n\n"
    f"4. Product Performance Analysis\n"
    f"   - Detailed technical benchmarks\n"
    f"   - Performance comparisons with data\n"
    f"   - Price-value analysis\n"
    f"   - Feature comparison matrices\n\n"
    f"5. Industry Trends & Future Outlook\n"
    f"   - Technology evolution roadmap\n"
    f"   - Market growth projections\n"
    f"   - Regulatory considerations\n"
    f"   - Future market scenarios\n\n"
    f"ANALYTICAL REQUIREMENTS:\n"
    f"1. Start analysis with concrete market size data\n"
    f"2. Use specific numbers and metrics throughout\n"
    f"3. Include data visualizations and tables\n"
    f"4. Build analysis from response data to market level\n"
    f"5. Include competitive positioning analysis\n"
    f"6. Support all claims with specific evidence\n\n"
    f"VISUALIZATION REQUIREMENTS:\n"
    f"- Market share charts\n"
    f"- Competitive positioning matrices\n"
    f"- Performance comparison tables\n"
    f"- Trend analysis graphs\n"
    f"- Segment distribution charts\n\n"
    f"CITATION REQUIREMENTS:\n"
    f"When presenting data or insights from the research learnings, use inline citations that reference the specific source. For example:\n"
    f"'The i9-14900K consumes approximately 30% more power than AMD alternatives [TechSpot]'\n"
    f"Do not create a new references section as sources are already provided.\n\n"
    f"<prompt>{prompt}</prompt>\n\n"
    f"Here are all the learnings from research:\n\n<learnings>\n{learnings_string}\n</learnings>"
    f"Here are the persona responses:\n\n<persona_responses>\n{persona_response}\n</persona_responses>"
    ...
    async def process_query(serp_query: SerpQuery) -> ResearchResult:
        async with semaphore:
            try:
                ...
                print(f"[deep_research] [process_query] result: {result}")
